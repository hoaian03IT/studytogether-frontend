...
export const TextQuiz = forwardRef(({ question, answer, audio, isCorrect, handleCheckResult, rd }, ref) => {
	...
	console.log({ type: "Quiztext", audio: audio, answer, question });
	...
	return (
		<div>
			<form onSubmit={handleSubmit} autoFocus>
				<h2 className='text-4xl font-bold mb-6 flex flex-col items-center justify-center gap-2'>
					{question} {audio && <Audio src={audio} show={!!audio} rd={rd} />}
				</h2>
				<input
				...
				{!isSubmitted && (
					<div className='flex justify-center'>
						<Button type='submit' size='lg' color='primary' variant='shadow' className='text-2xl'>
							Submit
						</Button>
					</div>
				)}
				{isSubmitted ? (
					isCorrect ? (
						<div className='w-full h-20 bg-success-200 text-success-600 text-4xl flex items-center justify-center gap-2 rounded-medium'>
							<FaRegThumbsUp className='size-10' /> Great job!
						</div>
					) : (
						<div className='w-full h-20 bg-danger-100 text-danger-600 text-4xl flex items-center justify-center gap-2 rounded-medium'>
							<FaRegSadCry className='size-10' /> The correct is:{" "}
							<strong className='font-bold underline'>{answer}</strong>
						</div>
					)
				) : (
					<div></div>

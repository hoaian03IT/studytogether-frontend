...
const HeaderLearnProgress = memo(function ({ page = "learn", title, courseId }) {
	const navigate = useNavigate();
	return (
		<div
			className={clsx(
				page === "speed-review" ? "bg-danger" : page === "classic-review" ? "bg-third" : "bg-primary",
			)}>
			<div className="text-2xl font-bold container py-2">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						{page === "speed-review" ? (
							<IoSpeedometerOutline className="size-8 text-white" />
						) : page === "classic-review" ? (
					...
					<div>
						<Tooltip
							content="Exit current session"
							className="bg-gray-800 text-white"
							placement="bottom"
							offset={2}
							radius="none"
							closeDelay={100}>
							<button onClick={() => navigate(pathname.courseParticipant.split(":")[0] + courseId)}>

...
const ListExamples = () => {
  ...
  const vocabularyQuery = useQuery({
    ...
    queryFn: async ({ queryKey }) =>
      await VocabularyService.fetchVocabulary(queryKey[1], user, updateUserState),
  ...
  const addExampleMutation = useMutation({
    ...
    onSuccess: ({ newExample }) => {
      ...
      clientQuery.setQueryData(
        [queryKeys.exampleByWord, params?.courseId, selectedWord.wordId],
        newExamples
      );
      clearFormSubmit();
    },
                      ...
                      <ul className="list-disc pl-4">
                        {exampleSuggestions.map((suggestion, index) => (
                          <li
                            ...
                            onClick={() => setFormValue({ ...formValue, example: suggestion })}
                          >
                            {suggestion}
                          </li>
                        ))}

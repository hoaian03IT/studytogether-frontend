...
function Payment() {
	...
	const coursePriceQuery = useQuery({
		...
		queryFn: async () => await CourseService.fetchCoursePrices(params?.courseId, user, updateUserState),
	...
	const fetchUrlVnPay = async () => {
		...
		const newWindow = window.open(
			data?.["vnpUrl"],
			"_blank",
			`width=${widthWindow},height=${heightWindow},left=${windowPosition[0]},top=${windowPosition[1]}},toolbar=no,menubar=no,scrollbars=yes,resizable=yes`,
		);
	...
	return isEnrollment ? (
		<Navigate to={pathname.courseInformation.split(":")[0] + params?.courseId} />
	) : (
			...
			<div className="bg-white p-6 rounded-lg shadow-lg lg:w-1/2 w-full">
				...
				<div className="space-y-4">
					<div className="flex items-center border-b pb-4">
						<img src={courseInfoQuery.data?.["image"]} alt="Course 1" className="w-16 h-16 rounded mr-4" />
						<div className="flex flex-col">
							...
							<div className="flex items-center">
								...
								<p className="ms-2 text-danger text-sm">
									-{coursePriceQuery.data?.["discount"] * 100}%
								</p>
							</div>
						</div>
					</div>
				...
				<div className="mt-6 border-t pt-4">
					...
					<div className="flex justify-between text-gray-600">
						...
						<span>
							{coursePriceQuery.data?.["currency"] === "USD"
								? USDollar.format(
										coursePriceQuery.data?.["price"] * coursePriceQuery.data?.["discount"],
								  )
								: VNDong.format(coursePriceQuery.data?.["price"] * coursePriceQuery.data?.["discount"])}
						</span>
					</div>
					<div className="flex justify-between text-xl font-semibold mt-4">
						...
						<span>
							{coursePriceQuery.data?.["currency"] === "USD"
								? USDollar.format(
										coursePriceQuery.data?.["price"] * (1 - coursePriceQuery.data?.["discount"]),
								  )
								: VNDong.format(
										coursePriceQuery.data?.["price"] * (1 - coursePriceQuery.data?.["discount"]),
								  )}
			...
			<div className="bg-white p-6 rounded-lg shadow-lg lg:w-1/2 w-full">
				...
				<div
					className={`overflow-hidden transition-all duration-300 ease-in-out ${
						selectedPaymentMethod ? "max-h-40 opacity-100" : "max-h-0 opacity-0"
					}`}>

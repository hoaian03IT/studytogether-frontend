...
import { Button, Modal, ModalContent, ModalFooter, ModalHeader, Popover, PopoverContent, PopoverTrigger } from "@nextui-org/react";
...
const OwnCourse = () => {
  ...
  // Fetch courses
  const { mutate: fetchOwnCourses, isLoading } = useMutation({
    mutationFn: () => CourseService.getOwnCourses(user, () => {}),
    onSuccess: (data) => {
      setCourses(data);
    },
    onError: (error) => {
  ...
  const { mutate: deleteCourse, isLoading: isDeleting } = useMutation({
    mutationFn: () => CourseService.deleteOwnCourse(selectedCourseId, user, () => {}),
    onSuccess: () => {
                ...
                <div key={formattedCourse.id} className="bg-white rounded-lg p-4">
                  ...
                  <div className="flex mt-4">
                      ...
                      <Popover>
                        ...
                        <PopoverContent>
                          <div className="flex flex-col py-2">
                            <button
                                                             onClick={() => navigate(`/edit-course/${item["course id"]}`)}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        navigate(`/course-information/${formattedCourse.id}`)
                      }
                    >
                      Start
                    </Button>

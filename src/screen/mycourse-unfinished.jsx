...
import {Button,Avatar,Modal,ModalContent,ModalFooter,ModalHeader,Popover,PopoverContent,PopoverTrigger,Progress,} from "@nextui-org/react";
...
const UnfinishedCourse = () => {
  ...
  // Fetch unfinished courses
  const { mutate: fetchCourses, isLoading: isFetching } = useMutation({
  ...
  const { mutate: deleteCourse, isLoading: isDeleting } = useMutation({
    mutationFn: () => CourseService.deleteOwnCourse(selectedCourseId, user, updateUserState),
    onSuccess: () => {
